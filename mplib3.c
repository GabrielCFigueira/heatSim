...
int inicializarMPlib(int capacidade_de_cada_canal, int ntasks) {
  ...
  channel_array    = (Channel_t**) malloc (sizeof(Channel_t*)*ntasks*ntasks);
	mutex_array = (pthread_mutex_t*) malloc( ntasks * ntasks * sizeof(pthread_mutex_t));
  ...
  if (channel_array == NULL || mutex_array == NULL || wait_for_free_space == NULL ||
 wait_for_messages == NULL) {
...
int receberMensagem(int tarefaOrig, int tarefaDest, void *buffer, int tamanho) {
  ...
  if(pthread_mutex_lock(&mutex_array[tarefaDest*number_of_tasks+tarefaOrig]) != 0) {
  ...
  while (!mess) {
    if(pthread_cond_wait(&wait_for_messages[tarefaDest*number_of_tasks+tarefaOrig], &mutex_array[tarefaDest*number_of_tasks+tarefaOrig]) != 0) {
  ...
  if(pthread_mutex_unlock(&mutex_array[tarefaDest*number_of_tasks+tarefaOrig]) != 0) {
...
int enviarMensagem(int tarefaOrig, int tarefaDest, void *msg, int tamanho) {
  ...
  if (channel_capacity >0) {
    while (leQueSize(channel->message_list) >= channel_capacity) {
      if(pthread_cond_wait(&wait_for_free_space[tarefaDest*number_of_tasks+tarefaOrig], &mutex_array[tarefaDest*number_of_tasks+tarefaOrig]) != 0) {
        fprintf(stderr, "\nErro ao esperar pela variável de condição\n");
        return -1;
      }
    }
